...
def prepare_feature_network_data(df_filled, target_assay, fingerprint_type, smiles_column='standardised_smiles', dataset_name=None):
    ...
    if len(binary_assays) > 0:
        ...
        # if there are no available binary assays, create empty features for concatenation with fingerprints
    ...
    # concatenate fingerprints and binary assay labels
    ...
    # only keep available indices, filter out missing values
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Evaluate XGBoost on molecular datasets (fingerprint-only) with 5-fold 70/10/20 split',
    ...
    parser.add_argument('--results_path', type=str,
                        default='experiments/results_XGB_sklearn', help='Results output directory')
    parser.add_argument('--n_splits', type=int,
                        default=5, help='Number of folds')
    parser.add_argument('--seed', type=int, default=42, help='Random seed')
    parser.add_argument('--n_jobs', type=int, default=12,
                        help='Number of CPU cores to use (default: 4, -1 for all cores)')
...
def main():
    ...
    # according to parameters, decide whether to impute
    if args.requires_imputation:
        print("Need to impute")
        # only read imputed CSV (if not exist, raise error)
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(
            os.path.dirname(os.path.dirname(current_dir)))
        imputed_data_dir = os.path.join(
            project_root, 'src', 'data', 'Imputed_data')
    ...
    for fold_id, (trainval_idx, test_idx) in enumerate(splits, start=1):
        ...
        per_fold_out = os.path.join(
            args.results_path,
            f"{args.dataset}_{args.fingerprint}_{args.target_assay}_XGB_fold{fold_id}_preds.csv"
        )
        if task_type == 'classification':
    ...
    metrics_out = os.path.join(
        args.results_path,
        f"{args.dataset}_{args.fingerprint}_{args.target_assay}_XGB_folds_metrics.csv"
    )
    metrics_df.to_csv(metrics_out, index=False)
    print(f"Saved evaluation metrics for each fold: {metrics_out}")

    # generate average summary (format: mean ± std)
    ...
    summary_out = os.path.join(
        args.results_path,
        f"{args.dataset}_{args.fingerprint}_{args.target_assay}_XGB_summary.csv"
    )
